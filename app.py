...
user_data = {}
graph_cache = {}       # {birthday_iso: {"data": {...}, "cached_at": datetime}}
image_cache = {}       # {img_id: bytes}
image_cache_order = [] # insertion-order list for cleanup
MAX_IMAGES = 60

# ──────────────────────────────────────────────────────────────
# Colors for 5 divination systems
# ──────────────────────────────────────────────────────────────
SYSTEMS = ["四柱推命", "算命学", "西洋占星術", "数秘術", "紫微斗数"]
COLORS = {
    "四柱推命":   "#4FC3F7",  # sky blue
    "算命学":     "#FFD54F",  # gold
    "西洋占星術": "#FF7043",  # orange (sun/fire)
    "数秘術":     "#66BB6A",  # green
    "紫微斗数":   "#AB47BC",  # purple
}
...
def generate_fortune_image(graph_data, birthday_iso):
    """Render 2-panel fortune chart and return PNG bytes."""
    current_year = datetime.now().year
...
def graph_image_thread(user_id, birthday):
    """Generate chart image and send as LINE ImageMessage."""
    try:
