...
# 九星ごとの社会的・集合的テーマ（LoveMEDOメソッド）
KYUSEI_SOCIAL_THEMES = {
    "一白水星": {
        "elements": "水・陰・流動・秘密・縁",
        "news_themes": [
            "水に関するニュース（河川・海・雨・水道）",
            "秘密の暴露・情報漏洩",
            "縁・別れ・つながりに関する話題",
            "名前に「水・海・川・雨・冬・北」が入る人物関連",
        ],
        "policy_themes": ["水資源・環境政策", "情報・プライバシー関連法"],
        "celeb_kanji": ["水", "海", "川", "冬", "北", "湖", "流"],
    },
    "二黒土星": {
        "elements": "土・陰・蓄積・忍耐・母性",
        "news_themes": [
            "土地・不動産・農業に関するニュース",
            "女性・母親・家族に関する話題",
            "地盤・地震・地滑りに注意が必要な日",
            "名前に「土・田・地・母・農」が入る人物関連",
        ],
        "policy_themes": ["農業政策", "国土・土地利用", "女性活躍政策"],
        "celeb_kanji": ["土", "田", "地", "里", "村", "農"],
    },
    "三碧木星": {
        "elements": "木・陽・発展・雷・声・始動",
        "news_themes": [
            "新しい動き・始動・発表に関するニュース",
            "SNS・メディアでの論争・炎上に関する話題",
            "若い世代・スタートアップ関連の話題",
            "名前に「木・樹・森・林・青・春・雷」が入る人物関連",
        ],
        "policy_themes": ["新法律・新制度の発表", "若者政策", "IT・スタートアップ支援"],
        "celeb_kanji": ["木", "樹", "森", "林", "青", "春", "雷", "翠"],
    },
    "四緑木星": {
        "elements": "木・陰・縁・信頼・風・成長",
        "news_themes": [
            "縁・出会い・婚姻に関するニュース",
            "風・大気・環境に関する話題",
            "信頼・契約・取引に関する動き",
            "名前に「緑・風・縁・葉・花・木・翠」が入る人物関連",
        ],
        "policy_themes": ["外交・条約", "環境・風力エネルギー政策"],
        "celeb_kanji": ["緑", "風", "葉", "花", "縁", "翠", "薫"],
    },
    "五黄土星": {
        "elements": "土・中央・変革・破壊・再生・強力",
        "news_themes": [
            "大きな変化・権力の交代・解体に関するニュース",
            "強いエネルギーの衝突・対立の表面化",
            "再生・復活・大規模な動きに関する話題",
            "名前に「五・中・央・力・剛・豪」が入る人物関連",
        ],
        "policy_themes": ["政権交代・大規模改革", "大型予算・国家プロジェクト"],
        "celeb_kanji": ["剛", "豪", "力", "強", "大", "央", "主"],
    },
    "六白金星": {
        "elements": "金・陽・権威・天・完成・断絶",
        "news_themes": [
            "権威・権力に関する動き（逮捕・解任・表彰）",
            "金融・株・経済に関するニュース",
            "天・空・航空に関する話題",
            "名前に「金・銀・剣・鋼・空・天・光」が入る人物関連",
        ],
        "policy_themes": ["金融政策・日銀", "航空・宇宙政策", "司法・断罪"],
        "celeb_kanji": ["金", "銀", "剣", "空", "天", "光", "晃"],
    },
    "七赤金星": {
        "elements": "金・陰・言葉・社交・享楽・収穫",
        "news_themes": [
            "言葉・発言・スピーチに関するニュース",
            "芸能・エンタメ・スキャンダルの話題",
            "食・飲食・グルメに関する動き",
            "名前に「赤・紅・悦・楽・歌・愛・美」が入る人物関連",
        ],
        "policy_themes": ["文化・芸術政策", "飲食・観光産業支援"],
        "celeb_kanji": ["悦", "楽", "歌", "愛", "美", "彩", "絵", "芸"],
    },
    "八白土星": {
        "elements": "土・陽・変革・継承・山・蓄積",
        "news_themes": [
            "山・登山・高地に関するニュース",
            "継承・引き継ぎ・世代交代に関する話題",
            "不動産・土地・資産に関する動き",
            "名前に「山・岳・峰・高・白・継・承」が入る人物関連",
        ],
        "policy_themes": ["不動産・住宅政策", "世代交代・相続制度", "山・地方創生"],
        "celeb_kanji": ["山", "岳", "峰", "高", "嶋", "嶺", "隆"],
    },
    "九紫火星": {
        "elements": "火・陽・名誉・情熱・美・頂点・スキャンダル",
        "news_themes": [
            "著名人のスキャンダル・名誉に関するニュース",
            "火・光・熱に関する話題（発火・照明・エネルギー）",
            "美・芸術・ファッションに関する動き",
            "名前に「火・炎・光・紅・美・華・輝」が入る人物関連",
        ],
        "policy_themes": ["エネルギー政策（火力・熱）", "文化・芸術振興", "著作権・知的財産"],
        "celeb_kanji": ["火", "炎", "光", "紅", "美", "華", "輝", "燈", "明"],
    },
}

def _gen_personalized_text(user, cat_sc, sys_scores, date_label, mode, extra_context="", social_theme_text="", day_kyusei_name="", month_kyusei_name=""):
    ...
    # 最低・最高スコアカテゴリ特定
    _cat_scores = {cat: cat_sc.get(cat, 5) for cat in ["全体運","金運","恋愛運","仕事運","健康運","対人運"]}
    weakest_cat = min(_cat_scores, key=_cat_scores.get)
...
・数秘ライフパス: {life_path}
・占術スコア（{mode}運勢値, 各1〜10）: {scores_str}
・最低スコアのカテゴリ: {weakest_cat}（{weakest_score}/10）
・最高スコアのカテゴリ: {strongest_cat}（{strongest_score}/10）
...
・social_prediction: 九星の象意から導かれる「今日/今月話題になりそうなテーマ」を2〜4個列挙。
  形式: 「〇〇に関するニュースや話題が出やすい」「名前に〇が入る人物の動向に注目」など。
  災害系は「〇〇に注意が必要な日」と濁す表現にする。
・energy_message: {day_kyusei_name}と{month_kyusei_name}のエネルギー×個人命式の共鳴（50文字以内）
・各カテゴリのmessage（50〜80文字）:
  - 個人の命式スコアと九星・干支の具体的な絡みを書く
  - スコアが低いカテゴリは「〇〇のエネルギーが個人の△△運と摩擦を起こしやすい時期。〇〇することで緩和できる」
  - スコアが高いカテゴリは「〇〇の気が命式の◇◇を後押しし、△△のテーマが動きやすい」
  - 断言しないが占術根拠を感じさせる
・lucky_explanation: ラッキー色・行動・アイテムの根拠を占術的に説明（60〜100文字）
  形式例: 「{day_kyusei_name}の五行属性と{month_kyusei_name}のエネルギーが重なる今日、あなたの命式の〇〇の気を高める○色が運気を底上げする。また{weakest_cat}の摩擦を、〇〇の行動で解消すると良い」
  ※必ず最低スコアカテゴリへの具体的アドバイスを含める

以下のJSON形式のみで回答。コードブロック不要：
{{
  "overall_message": "今日/今月の本質（20文字以内）",
  "social_prediction": ["話題になりそうなテーマ1", "話題になりそうなテーマ2", "話題になりそうなテーマ3"],
  "energy_message": "時代×個人の共鳴（50文字以内）",
  "scores": {{"overall": 5, "money": 5, "love": 5, "work": 5, "health": 5, "social": 5}},
  "details": {{
    "overall": {{"score": 5, "message": "九星×命式の流れ（50〜80文字）"}},
    "money": {{"score": 5, "message": "..."}},
    "love": {{"score": 5, "message": "..."}},
    "work": {{"score": 5, "message": "..."}},
    "health": {{"score": 5, "message": "..."}},
    "social": {{"score": 5, "message": "..."}}
  }},
  "lucky": {{
    "color": "ラッキーカラー（複数可）",
  ...
  "lucky_explanation": "ラッキーアイテム全体の占術的根拠（60〜100文字）",
...
def gen_daily(user):
    ...
    # 社会的テーマテキスト生成（日次）
    _day_themes = KYUSEI_SOCIAL_THEMES.get(today_kyusei, {})
    _month_themes_d = KYUSEI_SOCIAL_THEMES.get(month_kyusei, {})
    _news_d = _day_themes.get("news_themes", []) + _month_themes_d.get("news_themes", [])[:1]
    social_theme_text_d = "\n".join(f"・{t}" for t in _news_d[:4])
...
{YEAR_ENERGY_2026}"""
    if user.get("diagnosis_done"):
        personalized = _gen_personalized_text(user, cat_sc, s, date_str, "daily", extra_context=extra_context,
                                               social_theme_text=social_theme_text_d, day_kyusei_name=today_kyusei, month_kyusei_name=month_kyusei)
        if personalized:
...
def gen_monthly(user):
    ...
    if user.get("diagnosis_done"):
        ...
        month_meaning = KYUSEI_MEANINGS.get(month_kyusei, "")
        # 社会的テーマテキスト生成（月次）
        _month_themes_m = KYUSEI_SOCIAL_THEMES.get(month_kyusei, {})
        _news_m = _month_themes_m.get("news_themes", [])
        social_theme_text_m = "\n".join(f"・{t}" for t in _news_m[:3])
...
{YEAR_ENERGY_2026}"""
        personalized = _gen_personalized_text(user, cat_sc, sys_avg, month_str, "monthly", extra_context=extra_context,
                                               social_theme_text=social_theme_text_m, day_kyusei_name="", month_kyusei_name=month_kyusei)
        if personalized:
            _det_map = {"全体運":"overall","金運":"money","恋愛運":"love","仕事運":"work","健康運":"health","対人運":"social"}
            categories = {
                cat: {"score": cat_sc[cat],
...
def fmt_daily(data):
    ...
    if data.get('lucky_explanation'):
        lines.append("")
        lines.append(f"💡 {data['lucky_explanation']}")
    return "\n".join(lines)
