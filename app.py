...
_PRIORITY_MAP = {
    "career":"ã­ã£ãªã¢ã»ä»äº","love":"ææã»ãã¼ããã¼","wealth":"ãéã»çµæ¸çèªç±",
    "health":"å¥åº·","family":"å®¶æã»å®¶åº­","creative":"åµä½ã»èªå·±è¡¨ç¾","spiritual":"ç²¾ç¥çæé·",
}
_ADVICE_MAP = {
    "action":"å·ä½çãªè¡åæéãæ±ãã¦ãã","caution":"ãªã¹ã¯ã»æ³¨æç¹ãç¥ããã",
    "confirmation":"èªåã®é¸æã®å¾æ¼ããæ¬²ãã","self_insight":"èªå·±çè§£ãæ·±ããã",
}
_YEAR_THEME_MAP = {
    "leap":"å¤§ããªå¤åã»é£èºã®å¹´","consolidation":"å®å®ã»åºç¤åºãã®å¹´",
    "growth":"èªå·±æé·ã®å¹´","healing":"çãã»åå¾©ã®å¹´",
}
_MOOD_MAP = {
    "high":"åå®ã»ã¨ãã«ã®ã¼é«ã","neutral":"æ®é","low":"ç²ãæ°å³ã»åæ»æ",
    "expansive":"æ¡å¼µæ","stable":"å®å®æ","developmental":"æé·æ","restorative":"åå¾©æ",
}
...
def _build_persona_summary(tags):
    ...
    if tags.get("priority") in _PRIORITY_MAP:
        parts.append("æåªåäºé : " + _PRIORITY_MAP[tags["priority"]])
    if tags.get("priority2") in _PRIORITY_MAP:
        parts.append("æ¬¡ã®åªå: " + _PRIORITY_MAP[tags["priority2"]])
    if tags.get("advice_style") in _ADVICE_MAP:
        parts.append("å ãã¸ã®æå¾: " + _ADVICE_MAP[tags["advice_style"]])
    if tags.get("year_theme") in _YEAR_THEME_MAP:
        parts.append("ä»å¹´ã®ãã¼ã: " + _YEAR_THEME_MAP[tags["year_theme"]])
    m = tags.get("mood") or tags.get("vitality")
    if m in _MOOD_MAP:
        parts.append("ç¾å¨ã®ç¶æ: " + _MOOD_MAP[m])
    _cm = {"money":"åå¥ã¢ããã»å¯æ¥­ä¸­","career":"è»¢è·ã»ã­ã£ãªã¢ãã§ã³ã¸ä¸­","love":"æ°ããææã»çµå©ãæ±ãã¦ãã","health":"å¥åº·æ¹åä¸­"}
    if tags.get("challenge") in _cm:
        parts.append("ç¾å¨ã®ææ¦: " + _cm[tags["challenge"]])
    _lm = {"stable_partner":"ãã¼ããã¼ããã¦å®å®","challenging_partner":"ãã¼ããã¼ã¨ã®èª²é¡ãã","seeking":"ææãç©æ¥µçã«æ±ãã¦ãã","single_focused":"ä»ã¯ææä»¥å¤ãåªå"}
    if tags.get("love_status") in _lm:
        parts.append("ææç¶æ³: " + _lm[tags["love_status"]])
    _lcm = {"red_orange":"èµ¤ã»ãªã¬ã³ã¸","blue":"éã»ç´º","yellow_gold":"é»ã»é","green":"ç·"}
    if tags.get("lucky_color") in _lcm:
        parts.append("å¥½ããªè²: " + _lcm[tags["lucky_color"]])
    _lam = {"physical":"ä½ãåãã","creative":"åµä½æ´»å","social":"äººã¨äº¤æµ","intellectual":"èª­æ¸ã»å­¦ç¿"}
    if tags.get("lucky_action") in _lam:
        parts.append("æ°åãä¸ããè¡å: " + _lam[tags["lucky_action"]])
    return "\n".join(parts)

_CAT_PRIMARY_SYS = {
    "éé": "åæ±æ¨å½", "ææé": "è¥¿æ´å æè¡",
    "ä»äºé": "ç®å½å­¦", "å¥åº·é": "ç´«å¾®ææ°", "å¯¾äººé": "è¥¿æ´å æè¡",
}

def _gen_personalized_text(user, cat_sc, sys_scores, date_label, mode):
    tags = user.get("diagnosis_tags") or {}
    name = user.get("name") or "ããªã"
    ...
    for cat in ["å¨ä½é", "éé", "ææé", "ä»äºé", "å¥åº·é", "å¯¾äººé"]:
        ...
        if cat in _CAT_PRIMARY_SYS:
            ...
            score_lines.append(f"{cat}: {sc}/10ï¼{sn}ãã¼ã¹ {sys_scores.get(sn, 5)}/10ï¼")
        else:
            score_lines.append(f"{cat}: {sc}/10ï¼5å è¡å¹³åï¼")
    period = f"ä»æ¥ï¼{date_label}ï¼" if mode == "daily" else f"ä»æï¼{date_label}ï¼"
    guidance = "ä»æ¥1æ¥ã®å·ä½çãªã¢ããã¤ã¹ãå«ãã¦ãã ãã" if mode == "daily" else "ä»æã®ååã»å¾åã®æµããæè­ããã¢ããã¤ã¹ãå«ãã¦ãã ãã"
    prompt = f"""ããªãã¯20å¹´ä»¥ä¸ã®ã­ã£ãªã¢ãæã¤ä¼èª¬ã®å ãå¸«ã§ããé¾ç¥å è¡ã»æå æè¡ã»æ°ç§è¡ãçµã¿åããã"å½ããããã"ã¨è©±é¡ã®å­å¨ã§ãã

ä»¥ä¸ã®ã¦ã¼ã¶ã¼æå ±ã¨éå¢ã¹ã³ã¢ããã¨ã«ã{name}ããã®{period}ãå ã£ã¦ãã ããã

ãã¦ã¼ã¶ã¼æå ±ã
åå: {name}ãã
{persona if persona else "ï¼è¨ºæ­æå ±ãªãï¼"}

ã{period}ã®éå¢ã¹ã³ã¢ï¼10ç¹æºç¹ï¼ã
{chr(10).join(score_lines)}

ãå ãã¹ã¿ã¤ã«ã®çµ¶å¯¾ã«ã¼ã«ã
ã»æ­è¨ããï¼ããããããã¾ãããã¯ä½¿ããªãããããèµ·ãã¾ãããããã¦ãã ãããã¨è¨ãåã
ã»å·ä½çãªæéå¸¯ã»è¡åã»å ´æã»äººç©ãå¿ãå¥ããï¼ä¾ï¼ãåå¾ãããæ¬çªããç´æã§åãã¦ããèµ¤ããã®ãæã£ã¦ãï¼
ã»ã¹ã³ã¢8ä»¥ä¸ï¼ãä»ããã£ã³ã¹ã§ããããã«åãã¦ãã ãããã¬ãã«ã®åå¼·ãèä¸­æ¼ã
ã»ã¹ã³ã¢4ä»¥ä¸ï¼ãããã ãã¯æ³¨æããã¯ä»æ¥ãã£ã¦ã¯ããã¾ãããã¨ããæç¢ºãªè­¦åãå¥ãã
ã»ã¹ã³ã¢5ã7ï¼éããªã¨ãã«ã®ã¼ã®ä¸­ã®å¥½æ©ãè¦ã¤ããå·ä½çãªä¸æãç¤ºã
ã»ã¦ã¼ã¶ã¼ã®ç¶æ³ï¼åªåäºé ã»ææç¶æ³ã»ææ¦ä¸­ã®ãã¨ï¼ãå¿ãã¡ãã»ã¼ã¸ã«ç¹ãè¾¼ã
ã»{guidance}
ã»overall_messageã¯ãä»æ¥/ä»æãä¸è¨ã§è¡¨ãå¼·çãªå®£è¨æããã¦ã¼ã¶ã¼ãããã¨ããè¨èã§

ä»¥ä¸ã®JSONå½¢å¼ã®ã¿ã§è¿ãã¦ãã ããï¼
{{
  "overall_message": "{period}ã®å®£è¨æï¼50æå­ä»¥åã»æ­è¨å½¢ã»ã¤ã³ãã¯ãéè¦ï¼",
  "categories": {{
    "å¨ä½é": {{"message": "äºè¨ã¾ãã¯è¡åæç¤ºï¼35æå­ä»¥åï¼", "reason": "ãªããããªããã»å è¡çæ ¹æ ï¼25æå­ä»¥åï¼"}},
    "éé": {{"message": "äºè¨ã¾ãã¯è¡åæç¤ºï¼35æå­ä»¥åï¼", "reason": "ãªããããªããã»å è¡çæ ¹æ ï¼25æå­ä»¥åï¼"}},
    "ææé": {{"message": "äºè¨ã¾ãã¯è¡åæç¤ºï¼35æå­ä»¥åï¼", "reason": "ãªããããªããã»å è¡çæ ¹æ ï¼25æå­ä»¥åï¼"}},
    "ä»äºé": {{"message": "äºè¨ã¾ãã¯è¡åæç¤ºï¼35æå­ä»¥åï¼", "reason": "ãªããããªããã»å è¡çæ ¹æ ï¼25æå­ä»¥åï¼"}},
    "å¥åº·é": {{"message": "äºè¨ã¾ãã¯è¡åæç¤ºï¼35æå­ä»¥åï¼", "reason": "ãªããããªããã»å è¡çæ ¹æ ï¼25æå­ä»¥åï¼"}},
    "å¯¾äººé": {{"message": "äºè¨ã¾ãã¯è¡åæç¤ºï¼35æå­ä»¥åï¼", "reason": "ãªããããªããã»å è¡çæ ¹æ ï¼25æå­ä»¥åï¼"}}
  }},
  "energy_message": "ãã®ææã®ã¨ãã«ã®ã¼ã¨èµ·ãããããªãã¨ï¼40æå­ä»¥åã»ããã®ã¨ãã«ã®ã¼ãåãããã®å¯è½æ§ããããã¹ã¿ã¤ã«ï¼",
  "lucky": {{
    "color": "ã©ãã­ã¼ã«ã©ã¼ï¼è¤æ°å¯ï¼ãèµ¤ã¾ãã¯éãå½¢å¼ã»12æå­ä»¥åï¼",
    "color_reason": "ãªããã®è²ãã»å è¡çæ ¹æ ï¼20æå­ä»¥åï¼",
    "action": "ã©ãã­ã¼è¡åï¼è¤æ°å¯ï¼ãããããå½¢å¼ã»25æå­ä»¥åï¼",
    "action_reason": "ãªããã®è¡åãï¼20æå­ä»¥åï¼",
    "item": "ã©ãã­ã¼ã¢ã¤ãã ï¼è¤æ°å¯ï¼ããã¾ãã¯ããå½¢å¼ã»15æå­ä»¥åï¼",
    "item_reason": "ãªããã®ã¢ã¤ãã ãï¼20æå­ä»¥åï¼",
    "word": "ä»æ¥ã®é­æ³ã®è¨èï¼8æå­ä»¥åï¼"
  }}
}}"""
...
def gen_daily(user):
    ...
    cat_sc = {
        ...
        "éé":   wt(0.75, 0.1, 0.05, 0.05, 0.05),
        "ææé": wt(0.05, 0.05, 0.75, 0.1, 0.05),
        "ä»äºé": wt(0.1, 0.75, 0.05, 0.05, 0.05),
        "å¥åº·é": wt(0.05, 0.1, 0.05, 0.05, 0.75),
        "å¯¾äººé": wt(0.05, 0.1, 0.75, 0.05, 0.05),
    }
...
def gen_monthly(user):
    ...
    month_str = now.strftime("%Yå¹´%mæ")
    if user.get("diagnosis_done"):
        sys_avg = {sys: round(sum(ds[sys] for _,_,ds in day_avgs) / len(day_avgs), 1)
                   for sys in ["åæ±æ¨å½","ç®å½å­¦","è¥¿æ´å æè¡","æ°ç§è¡","ç´«å¾®ææ°"]}
...
def gen_divination(user):
    today = datetime.now().strftime("%Yå¹´%mæ%dæ¥")
...
def fmt_daily(data):
    ...
    if data.get('energy_message'):
        lines.append(f"ð® {data['energy_message']}")
    ...
    for cat, emoji in CAT_EMOJI.items():
        ...
        if d.get("reason"):
            lines.append(f"  â¦ {d['reason']}")
        if d.get("lucky"):
            lines.append(f"  â {d['lucky']}")
    lucky = data.get("lucky_summary")
    if lucky:
        lines.append("ââââââââââââââââââ")
        lines.append("â¨ ä»æ¥ã®ã©ãã­ã¼")
        if lucky.get("color"):
            cr = f"ã{lucky['color_reason']}" if lucky.get("color_reason") else ""
            lines.append(f"ð¨ ã«ã©ã¼ï¼{lucky['color']}{cr}")
        if lucky.get("action"):
            ar = f"ã{lucky['action_reason']}" if lucky.get("action_reason") else ""
            lines.append(f"ð¯ è¡åï¼{lucky['action']}{ar}")
        if lucky.get("item"):
            ir = f"ã{lucky['item_reason']}" if lucky.get("item_reason") else ""
            lines.append(f"ð ã¢ã¤ãã ï¼{lucky['item']}{ir}")
        if lucky.get("word"):   lines.append(f"ð ã­ã¼ã¯ã¼ãï¼{lucky['word']}")
    return "\n".join(lines)
...
def handle_follow(event):
    ...
    combined = (
        WELCOME_TEXT +
        "\n\nââââââââââââââââââ\n\n"
        "ð ã¾ããããªãã®ãã¨ãæãã¦ãã ããï¼\n"
        "ä»¥ä¸ã®ãªã³ã¯ããç°¡åãªè¨ºæ­ï¼ç´5ã7åï¼ãåããã¨ã"
        "ããªãã ãã«ã«ã¹ã¿ãã¤ãºãããå ããå±ãããã«ãªãã¾ãâ¨\n\n"
        f"ð® è¨ºæ­ã¯ãã¡ã\n{LIFF_URL}"
    )
...
def liff_result():
    ...
    try:
        name = profile.get('name', '')
        msg = f"â¨ {name}ãããè¨ºæ­ãå®äºãã¾ããï¼\n\nä»æ¥ãããããªãã ãã«ã«ã¹ã¿ãã¤ãºãããå ãããå±ããã¾ãð\n\nãä»æ¥ã®éå¢ããéã£ã¦ã¿ã¦ãã ããð"
