...
・energy_message: 「○○が共鳴」「○○が交差」など状態描写だけで終わらせない。必ず「だから今日は○○すると吉」「○○を意識すると流れが変わる」という行動結論で締める。30文字以内。例：「発信力が高まる日。情報発信や自己表現を積極的に」
・各カテゴリのmessage: 最大2文、合計40文字以内。1文目：今日の状態を口語で（例「お金の流れが整ってきている」「相手の本音が見えやすい日」）。2文目：具体的な行動（例「支払いや契約は今日中に動かして」「聞き役に回ると信頼が上がる」）。熟語の連続禁止。スコアや占術名は書かない。
...
{{
  ...
  "social_prediction": ["15文字以内の断言1", "15文字以内の断言2", "15文字以内の断言3"],
  "energy_message": "時代×個人の共鳴（50文字以内）",
  "scores": {{"overall": 5, "money": 5, "love": 5, "work": 5, "health": 5, "social": 5}},
  "details": {{
    "overall": {{"score": 5, "message": "[占術名]の流れでは[具体的意味]＋行動指針（50〜80文字）"}},
